...
def detect_patterns(
    *,
    ...
    memory_id: str,
    content: str,
    classify_fn: Callable[[str], Tuple[str, float, Any]],
    ...
    try:
        memory_type, confidence, _en_content = classify_fn(content)
        result = graph.query(
            """
            MATCH (m:Memory)
            WHERE m.type = $type
...
def score_semantic_edges(
    *,
    ...
    """Score semantic neighbor edges with LLM across 67 dimensions.

    Takes the output of link_semantic_neighbors (list of (id, cosine) pairs),
    fetches neighbor content from graph, calls nano to score across 8 sub-layers,
    and creates SCORE_* edges in FalkorDB.

    Returns the number of SCORE_* edges created.
    """
    ...
    if openai_client is None:
        logger.debug("Skipping edge scoring: no OpenAI client available")
        return 0

    from automem.enrichment.edge_scoring import create_score_edges, score_edges_with_llm

    # Fetch neighbor content from graph
    neighbor_ids = [nid for nid, _ in semantic_neighbors]
    cosine_map = {nid: score for nid, score in semantic_neighbors}
    ...
    try:
        ...
        logger.exception("Failed to fetch neighbor content for edge scoring")
        return 0

    neighbors_for_scoring: List[Dict[str, Any]] = []
    for row in getattr(result, "result_set", []) or []:
        if len(row) >= 2 and row[0] and row[1]:
            neighbors_for_scoring.append({
                "id": str(row[0]),
                "content": str(row[1]),
                "cosine": str(round(cosine_map.get(str(row[0]), 0.0), 3)),
            })

    if not neighbors_for_scoring:
        return 0

    # Call nano to score all edges in one batch
    edge_scores = score_edges_with_llm(
        openai_client=openai_client,
        source_id=memory_id,
        source_content=source_content,
        neighbors=neighbors_for_scoring,
    )

    if not edge_scores:
        return 0

    # Create SCORE_* edges in FalkorDB
    timestamp = utc_now_fn()
    created = create_score_edges(
        graph=graph,
